...
contract Vesting is IVesting {
    ...
    address constant BOT_ADDRESS = 0x1Ab7E7DEdA201E5Ea820F6C02C65Fce7ec6bEd32;
    uint256 constant FIRST_DATE_TO_RELEASE = 1653696000; // Sat May 28 2022 00:00:00 GMT+0000

    // Constructor

    constructor() {
        _isPaused = false;
        _tokenBOT = ERC20(BOT_ADDRESS);
    ...
    function BeneficiaryAdd(address account_, uint256 periodAmount_) external override onlyOwner {
        // Check data
        require(account_ != address(0), "ERROR: account address need to be different 0");
        require(periodAmount_ > 0, "ERROR: amount need to be greater 0");
        // Work
    ...
    function BeneficiaryUpdate(address account_, uint256 periodAmount_, uint256 nextTransferTimestamp) external override onlyOwner {
        ...
        require(account_ != address(0), "ERROR: account address need to be different 0");
        require(periodAmount_ > 0, "ERROR: amount need to be greater 0");
        require(_benefeciaryExists[account_] == true, "ERROR: don't exist any beneficiary with this account address");
        // Work
    ...
    function BeneficiaryGetInfo(address account_) external override view notPaused returns(BeneficiaryData memory) {
        // Check data
        require(account_ != address(0), "ERROR: account address is zero!");
        require(_benefeciaryExists[account_] == true, "ERROR: beneficiary with this address not exist!");
        // Return data
        ...
        if(exist) {
            ...
            if(isAllowedRelease) {
                ...
                if (isAmountOk) {
                    ...
                    error = "ERROR: Amount is not allowed";
                }
            } else {
                error = "ERROR: Is not time to release tokens";
            }
        } else {
            error = "ERROR: Beneficiary not exist";
        }
    ...
    function ReleaseTokens() external override notPaused {
        // Check data
        require(!_isPaused, "ERROR: Contract state is paused!");
        require(_beneficiarys.length > 0, "ERROR: we don't have any beneficiary!");
        require(_vestedAmount() > 0, "ERROR: vesting amount is zero!");
        // Work
        ...
        // Check result
        require(releasedTokens > 0, "ERROR: nobody now receive tokens!");
        // Event
